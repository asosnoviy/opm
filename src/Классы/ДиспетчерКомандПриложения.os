...
Процедура ДобавитьКомандуList(Знач Парсер)
	...
	Парсер.ДобавитьПараметрФлагКоманды(Команда, "-remote", "список пакетов хаба");
	Парсер.ДобавитьКоманду(Команда);
КонецПроцедуры
...
Процедура ОбновитьПакет(Знач ЗначенияПараметров) Экспорт
	...
	Если ЗначенияПараметров["-f"] <> Неопределено Тогда
		Установщик.УстановитьПакетИзАрхива(ЗначенияПараметров["-f"]);
	ИначеЕсли ЗначенияПараметров["-all"] Тогда
		Установщик.ОбновитьВсеУстановленныеПакетыИзОблака();
	Иначе
...
Процедура ВывестиСписокПакетов(ЗначенияПараметров)
	
	Если ЗначенияПараметров["-remote"] Тогда
		КэшПакетов = Новый КэшПакетовХаба;
		СписокПакетов = КэшПакетов.ПолучитьПакетыХаба(); 
	Иначе
		КэшПакетов = Новый КэшУстановленныхПакетов;
		СписокПакетов = КэшПакетов.ПолучитьУстановленныеПакеты(); 
	КонецЕсли;	
