...
Перем ПакетУжеСтавилсяКакЗависимость;
...
Процедура ОбновитьВсеУстановленныеПакетыИзОблака() Экспорт
	
	ПакетыХаба = Новый КэшПакетовХаба;
	УстановленныеПакеты = ПолучитьУстановленныеПакеты();
	СписокУстановленныхПакетов = УстановленныеПакеты.ПолучитьУстановленныеПакеты();

	Для Каждого УстановленныйПакет Из СписокУстановленныхПакетов Цикл
		Если ПакетыХаба.ЭтоПакетХаба(УстановленныйПакет.Ключ) Тогда
			СкачатьИУстановитьПакет(УстановленныйПакет.Ключ, Неопределено);
		КонецЕсли;	
	КонецЦикла;	
...
Процедура СкачатьИУстановитьПакет(Знач ИмяПакета, Знач ВерсияПакета)
	...
	// Если ПакетУжеСтавилсяКакЗависимость = Неопределено Тогда
	// 	ПакетУжеСтавилсяКакЗависимость = Новый Соответствие;
	// КонецЕсли;

	// Если ПакетУжеСтавилсяКакЗависимость.Получить(ИмяПакета) = Неопределено Тогда
	// 	ПакетУжеСтавилсяКакЗависимость.Вставить(ИмяПакета,Истина);
	// Иначе
	// 	Лог.Информация(СтрШаблон("Пакет %1 уже устанавливался. Пропускаю.",ИмяПакета));
	// 	Возврат;	
	// КонецЕсли;
